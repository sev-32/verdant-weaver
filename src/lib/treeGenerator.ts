...
export function generateTreeGeometry(params: TreeParams, seed: number = 1337): TreeGeometry {
  ...
  // Crown envelope
  const crownCenterY = height * 0.55;
  ...
  // ========================================
  // UNIFIED TUBE RENDERER
  // The trunk and every branch use the same tube function.
  // order=0 is the trunk (thickest, most segments).
  // ========================================
  function addTube(
    ...
    // More segments for trunk (order 0), fewer for twigs
    const tubeSeg = order === 0 ? 16 : Math.max(4, 10 - order * 2);
    const tubeRing = order === 0 ? 12 : Math.max(4, 8 - order);
    ...
    for (let i = 0; i <= tubeSeg; i++) {
      ...
      // Smooth taper along segment
      let rad = r0 + (r1 - r0) * Math.pow(t, 0.6);

      // Base flare (only for order 0, near ground)
      if (order === 0 && pos[1] < height * 0.08) {
        const flareT = 1 - pos[1] / (height * 0.08);
        rad *= 1 + (flare - 1) * flareT * flareT;
      }

      // Buttress (only trunk near ground)
      ...
      for (let j = 0; j <= tubeRing; j++) {
        ...
        // Buttress flares at base (trunk only, near ground)
        if (order === 0 && buttressStrength > 0 && pos[1] < height * 0.12) {
          const buttressPhase = angle * buttressCount * 0.5;
          const buttressEnv = Math.pow(Math.max(0, Math.cos(buttressPhase)), 3);
          const buttressT = Math.pow(1 - pos[1] / (height * 0.12), 2);
          rx += buttressStrength * buttressEnv * buttressT * r0 * 0.5;
          rz += buttressStrength * buttressEnv * buttressT * r0 * 0.5;
        }

        // Fluting (trunk only)
        if (order === 0 && flutingStrength > 0) {
          const flute = Math.sin(angle * flutingCount) * flutingStrength * 0.05 * rad;
          rx += flute;
          rz += flute;
        }

        // Bark furrows (stronger on trunk & main branches)
        const furrowScale = order <= 1 ? 1.0 : 0.3;
        const barkN1 = fbm2(angle * barkFurrowFreq + seed * 0.1, pos[1] * 3.5, 3);
        const barkN2 = fbm2(angle * barkFurrowFreq * 2.3 + 7.7, pos[1] * 6.1, 2);
        const furrow = (barkN1 * 0.7 + barkN2 * 0.3) * barkFurrowDepth * rad * 0.06 * furrowScale;
        ...
        // Color
        const furrowDark = 1 - barkN1 * barkFurrowDepth * 0.2 * furrowScale;
        const darkening = isDead ? 0.6 : (1 - order * 0.03);
        const heightDark = 1 - clamp(pos[1] / height, 0, 0.3) * 0.1;
        const cv = (rng() - 0.5) * barkColorVar * (order === 0 ? 1 : 0.5);

        let cr = trunkColor[0] * furrowDark * darkening * heightDark + cv;
        let cg = trunkColor[1] * furrowDark * darkening * heightDark + cv;
        let cb = trunkColor[2] * furrowDark * darkening * heightDark + cv;
  ...
  function addLeafCluster(pos: Vec3, dir: Vec3, order: number) {
    ...
    for (let i = 0; i < effectiveCount; i++) {
      ...
      const center: Vec3 = [
        ...
        pos[1] + r * sinTheta * Math.sin(phi) * 0.7 + r * 0.2,
        pos[2] + r * cosTheta,
      ];
  ...
  // ========================================
  // UNIFIED RECURSIVE BRANCH SYSTEM
  // order=0 is the trunk. It's just the thickest, straightest branch.
  // ========================================
  function growBranch(
    startPos: Vec3,
    startDir: Vec3,
    ...
    if (depth > 40 || length < 0.05 || radius < 0.001) return;
    if (order > maxOrder) return;

    const isDead = order > 0 && rng() < deadBranchRatio;
    const isBroken = order > 0 && rng() < breakProbability;

    // Direction modification based on order
    let dir: Vec3;
    if (order === 0) {
      // Trunk: mostly straight up with slight lean and crookedness
      dir = v3normalize([
        startDir[0] + (rng() - 0.5) * crookedness * 0.3,
        startDir[1],
        startDir[2] + (rng() - 0.5) * crookedness * 0.3,
      ]);
    } else {
      // Branches: gravity, phototropism, droop
      const droopAmount = branchDroop + droopIncrease * order;
      const gravBend = -gravityResponse * order * 0.06;
      dir = v3normalize([
        startDir[0] + (rng() - 0.5) * 0.1 + phototropism * 0.02,
        startDir[1] + gravBend - droopAmount + (isWillow ? -0.12 * order : 0),
        startDir[2] + (rng() - 0.5) * 0.1,
      ]);
    }
    ...
    // Build bezier curve for this segment
    const p0 = startPos;
    const p3: Vec3 = v3add(p0, v3scale(dir, effectiveLength));

    const curveMag = (order === 0 ? crookedness * 0.5 : branchCurvature) * effectiveLength;
    const p1 = v3lerp(p0, p3, 0.33);
    const p2 = v3lerp(p0, p3, 0.66);

    if (order === 0) {
      // Trunk wobble via crookedness
      p1[0] += (rng() - 0.5) * curveMag * 0.8;
      p1[2] += (rng() - 0.5) * curveMag * 0.8;
      p2[0] += (rng() - 0.5) * curveMag * 1.0;
      p2[2] += (rng() - 0.5) * curveMag * 1.0;
    } else {
      // Natural S-curve with gravity on branches
      const droopAmount = branchDroop + droopIncrease * order;
    ...
    // Taper: end radius
    const r1 = isBroken ? radius * 0.5 : radius * (order === 0 ? 0.55 : 0.3);

    // Draw the tube
    addTube(p0, p1, p2, p3, radius, r1, order, isDead);

    // Add leaves on terminal/near-terminal branches
    ...
    if (order === 0) {
      // TRUNK: spawn main branches along its length, then continue upward
      const branchZone = firstBranchHeight; // fraction of trunk below which no branches
      for (let i = 0; i < rawBranchCount; i++) {
        const hNorm = i / rawBranchCount;
        const tAttach = branchZone + hNorm * (0.9 - branchZone);
        ...
        const az = (i / rawBranchCount) * Math.PI * 2 + rng() * 0.5;
        ...
        const attachRad = r0InterpolateAt(radius, r1, tAttach);
        const branchRad = attachRad * 0.4 * (0.6 + 0.4 * (1 - hNorm));

        growBranch(attachPos, childDir, branchLen, branchRad, 1, depth + 1);
      }

      // Trunk continuation (apical leader): trunk extends itself
      // The trunk is one continuous branch that may fork or just keep going
      if (apicalDom > 0.2 && effectiveLength > 1.0) {
        const contDir = v3normalize([
          dir[0] + (rng() - 0.5) * crookedness * 0.2,
          1.0, // keeps going up
          dir[2] + (rng() - 0.5) * crookedness * 0.2,
        ]);
        const contLen = effectiveLength * 0.65 * apicalDom;
        const contRad = r1 * 0.9;
        if (contLen > 0.3 && contRad > 0.01) {
          growBranch(p3, contDir, contLen, contRad, 0, depth + 1);
        }
      }
    } else {
      // BRANCHES: spawn sub-branches
      ...
      for (let c = 0; c < childBudget; c++) {
        ...
        const childRad = r0InterpolateAt(radius, r1, tAttach) * radiusDecay * (0.5 + rng() * 0.5);

        growBranch(attachPos, childDir, childLen, childRad, order + 1, depth + 1);
      }

      // Apical continuation for branches
      if (rng() < apicalDom * 0.5 && order < maxOrder && !isDead) {
        const contDir = v3normalize(v3add(dir, [
          (rng() - 0.5) * 0.08 + phototropism * 0.03,
          isConifer ? 0.1 : 0.03,
          (rng() - 0.5) * 0.08,
        ]));
        growBranch(p3, contDir, effectiveLength * 0.55, r1 * 0.85, order, depth + 1);
      }
      ...
      if (rng() < forkProbability && order < maxOrder - 1) {
        ...
        growBranch(forkPos, forkDir, effectiveLength * 0.5, radius * 0.6, order, depth + 1);
      }
    }
  }

  // Helper: interpolate radius at a point t along a segment
  function r0InterpolateAt(startR: number, endR: number, t: number): number {
    return startR + (endR - startR) * Math.pow(t, 0.6);
  }

  // ========================================
  // GROW THE TREE: Start from root, order 0 = trunk
  // ========================================
  const trunkStartDir: Vec3 = v3normalize([
    restLean * 0.5,
    1.0,
    (rng() - 0.5) * restLean * 0.3,
  ]);

  // The trunk is just the first branch, growing upward from the ground
  growBranch(
    [0, 0, 0],           // starts at ground level
    trunkStartDir,        // grows upward
    height * 0.55,        // trunk segment length
    baseRadius,           // thickest radius
    0,                    // order 0 = trunk
    0                     // depth 0
  );

  // ========================================
  // ROOTS (surface roots radiating from base)
  // ========================================
  ...
  // ========================================
  // EPICORMIC SHOOTS (small sprouts on trunk)
  // ========================================
  if (epicormicDensity > 0) {
    const epicCount = Math.round(epicormicDensity * rawBranchCount * 0.5);
    for (let i = 0; i < epicCount; i++) {
      const y = height * (0.1 + rng() * 0.4);
      const az = rng() * Math.PI * 2;
      const pos: Vec3 = [Math.cos(az) * baseRadius * 0.3, y, Math.sin(az) * baseRadius * 0.3];
      const epicDir: Vec3 = v3normalize([Math.cos(az) * 0.6, 0.5, Math.sin(az) * 0.6]);
      const epicLen = height * 0.06 * (0.5 + rng() * 0.5);
      growBranch(pos, epicDir, epicLen, baseRadius * 0.025, maxOrder - 1, 0);
    }
  }
